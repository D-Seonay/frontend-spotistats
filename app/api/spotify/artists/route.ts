...
export async function GET(request: NextRequest) {
  try {
    ...
    let allArtists: any[] = []
    let nextUrl: string | null = "https://api.spotify.com/v1/me/top/artists?limit=50&time_range=long_term"
    ...
    while (nextUrl) {
      ...
      if (response.status === 401) {
        ...
        if (!retryResponse.ok) {
          return NextResponse.json({ error: "Failed to fetch artists after token refresh" }, { status: retryResponse.status })
        }
        const data = await retryResponse.json();
        allArtists = allArtists.concat(data.items);
        nextUrl = data.next;
      } else if (!response.ok) {
        return NextResponse.json({ error: "Failed to fetch artists from Spotify API", details: await response.text() }, { status: response.status })
      } else {
